...
with tab2:
    ...
    st.metric(label = "Total Civil Cases Without Future Docket Dates", value = civil_df[civil_df["Docket Date"] == ""].count().astype(str))
    st.dataframe(civil_df)

    #Get count of general civil cases without future docket dates,
    st.metric(label = "General Civil Cases Without Future Docket Dates", value = civil_df[civil_df["Docket Date"] == "" & civil_df["Case Type"] == "Civil"].count().astype(str))
    st.dataframe(civil_df[civil_df["Docket Date"] == "" & civil_df["Case Type"] == "Civil"])

    #Get count of tax cases without future docket dates,
    st.metric(label = "Tax Cases Without Future Docket Dates", value = civil_df[civil_df["Docket Date"] == "" & civil_df["Case Type"] == "Tax"].count().astype(str))
    st.dataframe(civil_df[civil_df["Docket Date"] == "" & civil_df["Case Type"] == "Tax"])

    #Get count of OLS cases without future docket dates,
    st.metric(label = "OLS Cases Without Future Docket Dates", value = civil_df[civil_df["Docket Date"] == "" & civil_df["Case Type"] == "OLS"].count().astype(str))
    st.dataframe(civil_df[civil_df["Docket Date"] == "" & civil_df["Case Type"] == "OLS"])

    #Get count of juvenile cases without future docket dates,
    st.metric(label = "Juvenile Cases Without Future Docket Dates", value = civil_df[civil_df["Docket Date"] == "" & civil_df["Case Type"] == "Juvenile"].count().astype(str))
    ...
    st.metric(label = "Criminal Cases Without Future Docket Dates", value = criminal_df[criminal_df["Docket Date"] == ""].count().astype(str))
